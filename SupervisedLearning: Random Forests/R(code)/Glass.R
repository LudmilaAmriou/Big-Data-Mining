...
# Definir le maximum de variables a prendre pour chaque division [10] avec crossval
library(caret)
train_control <- trainControl(method = "cv", number = 10)

# Definir les params du grid search de 1 a 10 == nbcolumns
grid <- expand.grid(mtry = 1:ncol(Glass))

# Faire grid search avec 10-fold cross-validation
RF_model <- train(Type ~ ., data = Glass, method = "rf", trControl = train_control, tuneGrid = grid)

# Afficher Nb optimal de vars dans chaque div
print(RF_model$bestTune)

# Random Forest2
RF2 <- randomForest(Type ~ ., data = Glass, mtry =RF_model$bestTune$mtry)
RF2
